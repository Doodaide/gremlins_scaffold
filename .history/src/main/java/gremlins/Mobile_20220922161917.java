...
public abstract class Mobile extends Entity{
    ...
    public int collision(App app) { // mob to block collision 
        // ! Left side collision
        int toReturn = 0;
        if(collides(this.x, this.y + 1, app) || // top left corner 
            collides(this.x, this.y + this.HEIGHT - 1, app)){ // bottom left corner, hit left 
            leftAV = false;
            toReturn = 1;
            //System.out.println("Left side");
        }

        else if(!collides(this.x, this.y + 1, app) &&
            !collides(this.x, this.y + this.HEIGHT - 1, app)){ // top left corner, hit left 
            leftAV = true;
            toReturn = 0;
        ...
        if(collides(this.x + 1, this.y, app) || 
            collides(this.x+this.WIDTH - 1, this.y, app)){ // top left corner, hit up
            upAV = false;
            toReturn = 1;
        ...
        if(collides(this.x+this.WIDTH, this.y + 2, app) || 
            collides(this.x + this.WIDTH, this.y + this.HEIGHT - 2, app)){// Top right corner, hit right 
            rightAV = false;
            toReturn = 1;
            //System.out.println("Right side");
        }

        else if(!collides(this.x+this.WIDTH, this.y + 2, app) &&
            !collides(this.x + this.WIDTH, this.y + this.HEIGHT - 2, app)){// Top right corner, hit right 
            rightAV = true;
            toReturn = 0;
