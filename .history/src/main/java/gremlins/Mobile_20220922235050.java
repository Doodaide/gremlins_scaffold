...
public abstract class Mobile extends Entity{
    ...
    public int collision(App app) { // mob to block collision 
        ...
        if(collides(this.getX() - 20, this.getY() + 2, app)){ // top left corner
            if(collides(this.getX() - 20, this.getY() + this.HEIGHT - 2, app)){ // bottom left corner, hit left 
                leftAV = false;
                toReturn = 1;
        ...
        else if(!collides(this.getX() - 20, this.getY() + 2, app) &&
            !collides(this.getX() - 20, this.getY() + this.HEIGHT - 2, app)){ // top left corner, hit left 
            leftAV = true;
            toReturn = 0;
    ...
    public boolean collides(int xCoord, int yCoord, App app) {
        ...
        for (Immobile i : app.mapToDraw) { // i = block
            ...
            if ((xCoord >= i.getX() && xCoord <= i.getX() + i.WIDTH)
                ...
                System.out.println();
                this.collisionBlock = i; //arr[dir]; // finds the object the mobile collided with. 
