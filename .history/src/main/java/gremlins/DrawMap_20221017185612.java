...
public class DrawMap {
    ...
    public void generateMap(ArrayList<Character[]> a, App app){
        for (Character[] j : a){
            for(Character k :j){
                if(k == 'X'){ // stone block
                    app.getMap().add(new Indestructable(xCounter, yCounter, this.stonewall));
                }
                    ...
                    ap.add(new Gremlin(xCounter, yCounter, this.gremlin, this.slime, this.frozenG, this.frozenS));
                }
