    ...
    public void fireballMechanics(){
        for(Fireball f: existingFireballs){ // checks through every fireball
            if( f!= null){
                ...
                if((collided == 1 || f.getDecaying()) && f.collisionBlock.getViable()){ // fireball hit something new (1st condition), or the fireball has hit something and is in a decay sequence
