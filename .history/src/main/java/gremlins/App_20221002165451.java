    ...
    */
    public void setup() {
        ...
        try {
            ...
            File f = new File("level0.txt"); //File f = new File(layout);
        ...
        allBalls = new ArrayList<ArrayList<SlimeBall>>(gArray.size());
        balls = new [gArray.size()]ArrayList<SlimeBall>;
        for(ArrayList<SlimeBall> a : allBalls){
            a = null;
            NPMobileEntities.addAll(a);
        }
    }
    ...
    public ArrayList<ArrayList<SlimeBall>> getAllBalls(){
    ...
    public void draw() {
        ...
        for(Gremlin g : gArray){
            if(g != null){
                ...
                if((g.getSlimeCoolDown() >= this.enemyCooldown) && (g.getX() % 20 == 0 || g.getY() == 0)){
                    allBalls.add(g.shootSlimeBall());
                    g.setSlimeCooldown(0);
                }
                else if(g.getSlimeCoolDown() <= this.enemyCooldown){
                    g.setSlimeCooldown(g.getSlimeCoolDown() + 1);
                }
                g.slimeballMechanics(this);
                g.draw(this);
                
            }
        }
        System.out.println(this.allBalls.size());
