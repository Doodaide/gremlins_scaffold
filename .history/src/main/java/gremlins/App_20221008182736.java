    ...
    */
    public void setup() {
        ...
        try {
            ...
            this.frozenS = loadImage(this.getClass().getResource("Powerup_Freeze.png").getPath().replace("%20", " "));
        ...
        for (Character[] j : mapContents){
                for(Character k :j){
                    ...
                    if(k == 'X'){ // stone block
                        ...
                        gArray.add(new Gremlin(xCounter, yCounter, this.gremlin, this.slime, this.frozenG, ));
                    }
    ...
    public void powerUpUpdates(){
        if(p != null){
            if(!this.powerUpSpawned && p.getConsumed() && this.getFreezeActive()){ // consumed and not on screen 
                System.out.println("the powerup has been consumed and not on screen");
