    ...
    */
    ...
    public boolean fireballHit(){
        for (Immobile i : mapToDraw){
            if(i.getClass().getName().equals("gremlins.Destructable"){ // encountered a destructable block
                if(fireballHitObject(i)){

                }
            }
            else if(i.getClass().getName().equals("gremlins.Indestructable")){ // encountered a stone border
                ;
            }
        }
    }
    
    public boolean fireballHitObject(Entity a){
        if((a.getX() == f.getX() + 20 && a.getY() == f.getY()) || // hit left of block
        (a.getX() == f.getX() && i.getY() <= f.getY() + 20) || // hit top of block
    ...
    public void draw() {
        ...
        if(f != null){
            ...
            if(collided == 1){ // fireball hit something
                for (Immobile i : mapToDraw) { // i = block
                    //System.out.println("checked " + i.getClass().getName());
