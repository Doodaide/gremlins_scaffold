    ...
    */
    ...
    public boolean fireballHit(int x){ // block
        for (int i = 0; i < mapToDraw.size(); i++){
            ...
            if(mapToDraw.get(i) == existingFireballs.get(existingFireballs.size() - 1).collisionBlock && mapToDraw.get(i).getClass().getName().equals("gremlins.Destructable")){ // encountered a destructable block
    ...
    public void draw() {
        ...
        fill(0,0,0);
        w.draw(this);
        System.out.println(existingFireballs.size());
        //while(existingFireballs.remove(null));
        for (Fireball f : existingFireballs){
            System.out.println(f);
            if(f != null){  
                int collided = f.draw(this);
                if(collided == 1 || decaying){ // fireball hit something and correct direction 
                    //System.out.println("Collided");
                        ...
                        if(f.getFrameCounter() == 16){
                            ...
                            System.out.println("Collsiion block: " + mapToDraw.indexOf(f.collisionBlock));
                            if(mapToDraw.get(mapToDraw.indexOf(f.collisionBlock)) != null){
                                if(fireballHit(0)){
                            ...
                            System.out.println(f + f.getFrameCounter()rameCounter);
