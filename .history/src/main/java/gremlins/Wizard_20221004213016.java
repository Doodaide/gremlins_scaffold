...
public class Wizard extends Mobile{
    ...
    public void setExistingFireballs(ArrayList<Fireball> arr){
        this.existingFireballs = arr;
    }
    ...
    public Fireball shoot(App app){
        if((this.getX() % 20 == 0 || this.getY() % 20 == 0) && this.getManaCoolDown() >= app.getWizardCooldown()){
            this.setManaCooldown(0);
            this.existingFireballs.add(new Fireball(this.getX(), this.getY(), this.imgProjectile));
            return existingFireballs.get(existingFireballs.size() - 1); // returns newest added fireball
        }
        return null;
    }
