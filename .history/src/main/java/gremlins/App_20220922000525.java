    ...
    */
    ...
    public boolean fireballHit(){
        for (Immobile i : mapToDraw){
            ...
            if(i == f.collisionBlock && i.getClass().getName().equals("gremlins.Destructable")){ // encountered a destructable block
    ...
    public boolean fireballHitObject(Entity i){
        if((i.getX() == f.getX() || i.getY() == f.getY())  //|| // hit left of block
            //(i.getX() == f.getX() && i.getY() == f.getY() + 20) || // hit top of block
            //(i.getX() - 20 == f.getX() && i.getY() + 20 == f.getY()) //|| // hit right of block
    ...
    public void draw() {
        ...
        if(f != null){
            ...
            if(collided == 1){ // fireball hit something and correct direction 
                if((w.leftAV && f.direction == 3 && w.direction == 3)||
                    (w.upAV && f.direction == 0 && w.direction == 0) || 
                    (w.rightAV && f.direction == 1 && w.direction == 1)||
                    (w.downAV && f.direction == 2 && w.direction == 2)){
                        System.out.println("shot in the right direction");
                        fireballHit();
