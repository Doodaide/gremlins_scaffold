    ...
    public int collision(App app) {
        ...
        if (collides(this.x, this.y+velMult, app) ||
            ...
            collides(this.x+this.WIDTH-1, this.y+this.HEIGHT-velMult-1, app)) {
                System.out.println("hit bro 1");
        }
        if (collides(this.x+velMult, this.y, app) ||
            collides(this.x+this.WIDTH-1-velMult, this.y, app) ||
            collides(this.x+velMult, this.y+this.WIDTH-1, app) ||
            collides(this.x+this.WIDTH-1-velMult, this.y+this.WIDTH-1, app)) {
                System.out.println("hit bro 2");
        }
        return 0;
    }

    public boolean collides(int xCoord, int yCoord, App app) {
        if (xCoord > app.w.getX() && xCoord < app.w.getX() + app.w.WIDTH && yCoord > app.w.getY() && yCoord < app.w.getY()+Player.HEIGHT)
        /*
