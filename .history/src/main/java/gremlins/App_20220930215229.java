    ...
    public void draw() {
        ...
        for (int i = 0; i < gArray.size(); i++) {
            if(gArray.get(i) != null){
                if(gArray.get(i).getFrameCounter() < 20){
                    gArray.get(i).tick(this);
                    gArray.get(i).incFrameCounter();
                }
                
                else if(gArray.get(i).getFrameCounter() >= 20){
                    System.out.println("reset counter");
                    gArray.get(i).setFrameCounter(0);
                    gArray.get(i).gremlinAI(this);
                    
                    int toGo = gArray.get(i).getDirection();
                    if(gArray.get(i).getFrameCounter() == 0){
                        toGo = this.gremlinAI(this);
