...
public abstract class Mobile extends Entity{
    ...
    public int collision(App app) { // mob to block collision 
        ...
        if(collides(this.getX(), this.getY() + 2, app) || // top left corner 
            collides(this.x, this.getY() + this.HEIGHT - 2, app)){ // bottom left corner, hit left 
            leftAV = false;
            toReturn = 1;
            ...
            !collides(this.getX(), this.y + this.HEIGHT - 2, app)){ // top left corner, hit left 
            leftAV = true;
            toReturn = 0;
        ...
        if(collides(this.getX() + 2, this.y, app) || // top left corner, hit up
            collides(this.getX()+this.WIDTH - 2, this.y, app)){ // top right cornder hit up 
            upAV = false;
            toReturn = 1;
            System.out.println("up 0");
        }

        else if(!collides(this.getX() + 2, this.y, app) && 
            !collides(this.getX()+this.WIDTH - 2, this.y, app)){ // top left corner, hit up
            upAV = true;
            toReturn = 0;
        ...
        if(collides(this.getX()+this.WIDTH, this.y + 2, app) || // Top right corner, hit right 
            collides(this.getX() + this.WIDTH, this.y + this.HEIGHT - 2, app)){
            rightAV = false;
            toReturn = 1;
            System.out.println("right 1");
        }

        else if(!collides(this.getX() +this.WIDTH, this.y + 2, app) && // Top right corner, hit right 
            !collides(this.getX() + this.WIDTH, this.y + this.HEIGHT - 2, app)){
            rightAV = true;
            toReturn = 0;
        ...
        if(collides(this.getX() + 2, this.y + this.HEIGHT, app) || // Bottom left corner, hit down
            collides(this.getX() + this.WIDTH - 2, this.y + this.HEIGHT, app)){ // Bottom right corner, hit down
            downAV = false;
            toReturn = 1;
        ...
        else if(!collides(this.getX() + 2, this.y + this.HEIGHT, app) && 
            !collides(this.getX() + this.WIDTH - 2, this.y + this.HEIGHT, app)){// Bottom left corner, hit down
            downAV = true;
            toReturn = 0;
    ...
    public boolean collides(int xCoord, int yCoord, App app) {
        ...
        for (Immobile i : app.mapToDraw) { // i = block
            ...
            if ((xCoord > i.getX()-1 && xCoord < i.getX() + i.WIDTH+1)
                &&
                (yCoord > i.getY()-1 && yCoord < i.getY() + i.HEIGHT+1)){
