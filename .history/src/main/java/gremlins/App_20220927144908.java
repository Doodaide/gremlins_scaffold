    ...
    public boolean fireballHit(Fireball f){ // block
        for (int i = 0; i < mapToDraw.size(); i++){
            ...
            if(mapToDraw.get(i) != null){
                if(mapToDraw.get(i) == f.collisionBlock && 
    ...
    public void draw() {
        ...
        for (Fireball f : existingFireballs){
            ...
            if(f != null){  
                ...
                if((collided == 1 || f.getDecaying())){ // fireball has hit something or the fireball is decaying
                    if(f.collisionBlock.getViable()){
                        System.out.println("viable block hit");
                        f.setDecaying(true); // set decay state to true
                    ...
                    System.out.println("collided, but not valid block");
                    continue;

                }
                    } // fireball hit something and correct direction 
        ...
        */
        ...
        UpdateArrayList<Fireball> updater = new UpdateArrayList<Fireball>();
        this.existingFireballs = updater.update(existingFireballs);
        updater = null;
        UpdateArrayList<Gremlin> updater = new UpdateArrayList<Gremlin>()
        ...
        for (Mobile entity : this.NPMobileEntities) {
            if(!entity.getClass().getName().equals("gremlins.Fireball") && !entity.getClass().getName().equals("gremlins.Gremlin") ){
                temp2.add(entity);
            }
        }
