...
public abstract class Mobile extends Entity{
    ...
    public int collision(App app) {
        // ! Left side collision
        if(collides(this.x - 1, this.y, app) || collides(this.x - 1, this.y + this.HEIGHT, app)){ // top left corner, hit left 
            leftAV = false;
            System.out.println("Left side");
        }

        else if(!collides(this.x - 1, this.y, app) && !collides(this.x - 1, this.y + this.HEIGHT, app)){ // top left corner, hit left 
            leftAV = true;
            System.out.println("TL FREE LEFT");
        }

        // ! Upper side collision
        if(collides(this.x, this.y - 1, app) || collides(this.x+this.WIDTH, this.y - 1, app)){ // top left corner, hit up
            upAV = false;
            System.out.println("TL H U");
        }

        else if(!collides(this.x, this.y - 1, app) && !collides(this.x+this.WIDTH, this.y - 1, app)){ // top left corner, hit up
            upAV = true;
            System.out.println("TL FREE UP");
        }

        // ! Right side collision 
        if(collides(this.x+this.WIDTH + 1, this.y, app) || collides(this.x + this.WIDTH + 1, this.y + this.HEIGHT, app)){// Top right corner, hit right 
            rightAV = false;
            ;
        }

        else if(!collides(this.x+this.WIDTH + 1, this.y, app) && !collides(this.x + this.WIDTH + 1, this.y + this.HEIGHT, app)){// Top right corner, hit right 
            rightAV = true;
            ;
        }

        // ! Lower side collision 
        if(collides(this.x, this.y + this.HEIGHT + 1, app) || collides(this.x + this.WIDTH + 1, this.y + this.HEIGHT + 1, app)){// Bottom left corner, hit down
            downAV = false;
            ;
        }

        else if(!collides(this.x, this.y + this.HEIGHT + 1, app) && !collides(this.x + this.WIDTH + 1, this.y + this.HEIGHT + 1, app)){// Bottom left corner, hit down
            downAV = true;
            ;
        }
