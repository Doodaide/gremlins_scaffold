...
/**
 * BigTest
 * This test suite tests a whole lot of different interactions that 
 * are not testable without running an applet (or I'm just not smart enough to do so)
 * 
 */
public class BigTest {
   ...
   public void wizardStaticTest(){
        ...
        // now try to collide with something on the left 
        // No collision should be detected, so a 0 should be returned
        assertEquals(0, app.getWizard().leftCollision(app));
        // now try to collide with something on the right 
        assertEquals(0, app.getWizard().rightCollision(app));
        
        // Now try the invalid collision commands 
        // This is when the direction and collision direction do not match
        app.getWizard().setDirection(0);
        assertEquals(0, app.getWizard().leftCollision(app));
        assertEquals(0, app.getWizard().rightCollision(app));
        assertEquals(0, app.getWizard().downCollision(app));

        app.getWizard().setDirection(1); 
        assertEquals(0, app.getWizard().upCollision(app));

        app.delay(1000);

        // Test the draw method's teleport functionality 
        app.noLoop();
        app.getWizard().setTeleported(true);
        app.getWizard().setTpCooldown(999);
        app.getWizard().draw(app);
        assertEquals(998, app.getWizard().getTpCooldown());
    }
   ...
   public void wizardImmobileInteractionTest(){
        ...
        // Test if the powerup has been nulled 

        // Freeze gremlins again (for now), as we must test Kamehameha 
        app.setFreezeActive(true); 

        while(app.getPowerUp() != null){
            if(app.getPowerUp().getCooldown() == 0){
                assertFalse(app.getPowerUpAvailable());
                assertFalse(app.getFreezeActive());
                assertnull(app.getPowerUp();)
                break;
            }
            app.redraw();
        }
   ...
   public void slimeHitWizardTest(){
        ...
        // Save a reference to that slimeball
        SlimeBall s = app.getGArray().get(0).getAvailableSlimeballs().get(0);
        app.noLoop();

        // Slowly increment the slimeball to hit the wizard. 
        System.out.println("Start of loop");
        while(true){
            if(s.collisionEntity != null){
                // Check that the entity the slimeball has hit is in fact the wizard
                assertEquals(app.getWizard(), s.collisionEntity);
                System.out.println(app.getWizard().getX() + " " + app.getWizard().getY());
                System.out.println(" Wizard inside loop: " + app.getWizard().getAlive());
   ...
   @Test 
   public void directionalKamehamehaTest(){
        /*
         * This testcase is meant to test the other three cardinal directions that 
         * the Kamehameha beam can fire in. Using the same method of assessing 
         * whether there are any more destructable blocks in the line of sight of the 
         * wizard after the beam has been fired, the up, down and right directions 
         * will be tested. The beam charges won't be decremented 
         * as the decrementation is handled in the keypress area. 
         * This was put there to stop spamming, and punish impatience 
         * However, now I can't test it whoops. 
         */
        ...
        app.setFreezeActive(true); 
        app.loop();
        app.getWizard().setX(280);
        app.getWizard().setY(240);
        ...
        /******************** */
        // Left direction test 
        Kamehameha k1 = new Kamehameha(app.getWizard().getX(), app.getWizard().getY(), app.hBeam);

        // Check not null
        assertNotNull(k1);

        // The beam shouldn't be drawing yet, as there has been no loop
        assertFalse(k1.getDrawing());
        app.loop();
        app.delay(1000);
        // ! The beam will not consistently appear on screen for some reason
        /*
        ...
        */
        ...
        app.delay(1000);
        // Count how many destructables are in the same row and in the direction the wizard is pointing in 
        int leftCount = 0; // should end up at 0 after the beam has fired
        for (int i = app.getWizard().getX(); i >= App.LEFTBOUND; i-= 20) {
            for(Immobile I : app.getMap()){
                if(I != null){
                    if(I.getY() == app.getWizard().getY() && I.getX() < app.getWizard().getX() && I.getClass().getName().equals("gremlins.Destructable")){
                        leftCount += 1;
        ...
        app.delay(1000);
        assertEquals(0, leftCount); // check that it is equal to 0, as there should be no more destructable blocks left 
        ...
        Kamehameha k2= new Kamehameha(app.getWizard().getX(), app.getWizard().getY(), app.vBeam);

        // Check not null
        assertNotNull(k2);

        // The beam shouldn't be drawing yet, as there has been no loop
        assertFalse(k2.getDrawing());
        app.loop();
        app.delay(1000);
        // ! The beam will not consistently appear on screen for some reason
        /*
        ...
        */
        ...
        app.delay(1000);
        // Count how many destructables are in the same row and in the direction the wizard is pointing in 
        int downCount = 0; // should end up at 0 after the beam has fired
        for (int i = app.getWizard().getY(); i < App.LOWERBOUND; i += 20) {
            for(Immobile I : app.getMap()){
                if(I != null){
                    if(I.getX() == app.getWizard().getX() && I.getY() > app.getWizard().getY() && I.getClass().getName().equals("gremlins.Destructable")){
                        downCount += 1;
                        break;
                    }
                }
        ...
        app.delay(1000);
        assertEquals(0, downCount); // check that it is equal to 0, as there should be no more destructable blocks left
        ...
        Kamehameha k3= new Kamehameha(app.getWizard().getX(), app.getWizard().getY(), app.vBeam);

        // Check not null
        assertNotNull(k3);

        // The beam shouldn't be drawing yet, as there has been no loop
        assertFalse(k3.getDrawing());
        app.loop();
        app.delay(1000);
        // ! The beam will not consistently appear on screen for some reason
        /*
        ...
        */
        ...
        app.delay(1000);
        // Count how many destructables are in the same row and in the direction the wizard is pointing in 
        int upCount = 0; // should end up at 0 after the beam has fired
        for (int i = app.getWizard().getX(); i < App.UPPERBOUND; i -= 20) {
            for(Immobile I : app.getMap()){
                if(I != null){
                    if(I.getX() == app.getWizard().getX() && I.getY() < app.getWizard().getY() && I.getClass().getName().equals("gremlins.Destructable")){
                        upCount += 1;
        ...
        app.delay(1000);
        assertEquals(0, upCount); // check that it is equal to 0, as there should be no more destructable blocks left

   }

   @Test
   public void slimeHitFireballTest(){
        App app = new App();
        PApplet.runSketch(new String[] { "App" }, app);
        app.delay(500);
        // Freeze the gremlins first so we can relocate the wizard  
        ...
        // Turn the wizard right 
        app.getWizard().right();
        app.setFreezeActive(false);
        app.delay(10);
        app.noLoop();

        // Fire a slimeball at the wizard 
        Gremlin g = app.getGArray().get(0);
        g.shootSlimeBall();

        // and fire a fireball at the slime 
        app.getWizard().shoot();
        Fireball f = app.getWizard().getExistingFireballs().get(0);
        app.redraw();

        // Check that they have actually fired
        assertEquals(1, g.getAvailableSlimeballs().size());
        assertEquals(1, app.getWizard().getExistingFireballs().size());

        // Slowly increment until they collide 
        while(app.getWizard().getAlive()){
            if(f.collisionEntity != null){

                // Check that the collision entity is correct 
                assertEquals(g.getAvailableSlimeballs().get(0), f.collisionEntity);
                break;
            }
            app.redraw();
        }
