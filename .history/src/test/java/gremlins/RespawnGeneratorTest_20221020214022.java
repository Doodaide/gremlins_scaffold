...
public class RespawnGeneratorTest {
    ...
    public void method(){
         ...
         */
         ...
         // This statement tests that no block has the same x and y coords as 
         // our new set of coords. 
        for (int i = 0; i < app.getMap().size(); i++) {
            boolean statement = (app.getMap().get(i).getX() == output[0])
                             && (app.getMap().get(i).getY() == output[1]);
