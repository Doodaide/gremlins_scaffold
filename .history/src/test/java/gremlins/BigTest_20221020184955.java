...
public class BigTest {
    ...
    public void wizardImmobileInteractionTest(){
        ...
        app.noLoop();
        app.getWizard().shoot(); // ! Problematic line 
        app.delay(20);
        app.draw();
        ...
        // Freeze gremlins again (for now), as we must test the exit portal
        app.setFreezeActive(true); 
        ...
        // ! Check that lives are maintained 
        ...
        assertEquals(actualMapContents, app.getMap().size());
        for (int i = 0; i < app.getMap().size(); i++) {
            Immobile expected = app.getMap().get(i);
            Immobile actual = actualMapContents.get(i);
